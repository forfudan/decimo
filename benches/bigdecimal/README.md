...
Each op is exercised at working precisions chosen per op — 100, 1000 and
10000 for the arithmetic; 100 and 1000 for `exp`, `ln` and `root`; 100 alone
for the precision-insensitive `comparison`, `from_string` and `to_string`.
`--precisions` overrides all of them. The report shows one timings table per
`(op, precision)`.
