...
Measured against libmpdec directly (see `docs/benchmarks.md`, regenerated by
`pixi run benchdoc`), three things stand out:

- **`x += y` in place is 1.38x *faster* than libmpdec** (6.3 ns against
  8.7 ns). The arithmetic is not the problem; allocation is. That is also the
  clearest estimate yet of what inline storage is worth.
- **`x -= y` in place is 5.1x slower** (43.8 ns against 8.6 ns), because
  `subtract_inplace()` builds a whole negated copy of its right operand just
  to flip a sign. `x *= y` is 2.4x slower.
- **`BigDecimal` multiply loses at 100 000 digits** (4.27 ms against 2.72 ms)
  while winning at 1 000 and 10 000 (3.6x and 2.8x). The NTT lives in
  `bigint/` and is reachable only from base-2^32 multiplication; `BigUInt` is
  base-10^9 and still tops out at Toom-3. The same product through `BigInt`
  takes 1.14 ms -- 3.7x faster than through `BigDecimal`, on identical
  numbers. Wiring a transform into the base-10^9 path would take that column.
